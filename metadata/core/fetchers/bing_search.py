...
class BingSearchFetcher(BaseFetcher):
    """Fetcher using Bing Search API as alternative to Google CSE."""
    ...
    def can_fetch(self, tool_name: str) -> bool:
        """Bing Search can fetch any tool if API key is available."""
    ...
    def fetch(self, tool_name: str) -> Optional[UnifiedMetadata]:
        """Fetch metadata using Bing Search API."""
        if not self.api_key:
            logger.warning("Bing Search API key not available")
            return None
        
        try:
            logger.info(f"BingSearchFetcher: Searching for {tool_name}")
            
            # Search for tool information
            search_results = self._bing_search(tool_name)
            
            if not search_results:
                logger.warning(f"No Bing Search results found for {tool_name}")
                return None
            ...
            logger.error(f"Error in Bing Search for {tool_name}: {e}")
            return None
    
    def _bing_search(self, tool_name: str) -> List[Dict[str, Any]]:
        """Perform Bing Search for tool information."""
        try:
            # Search parameters
            params = {
                'q': f"{tool_name} programming language framework library documentation",
                'count': 10,
                'offset': 0,
                'mkt': 'en-US',
                'safesearch': 'moderate'
            ...
            if 'webPages' in data and 'value' in data['webPages']:
                for item in data['webPages']['value']:
                    results.append({
                        'title': item.get('name', f"{tool_name.title()} Resource"),
            ...
            if 'news' in data and 'value' in data['news']:
                for item in data['news']['value'][:3]:  # Limit to 3 news items
                    results.append({
                        'title': f"{tool_name.title()} - {item.get('name', 'News')}",
    ...
    def _generate_description_from_results(self, tool_name: str, results: List[Dict[str, Any]]) -> str:
        """Generate description from Bing Search results."""
        import random
        ...
        if results:
            ...
            if 'web_search' in source_types:
                base_description += " Bing Search provides comprehensive web results and detailed information about this technology."
            if 'news' in source_types:
                base_description += " Recent news and updates indicate active development and community engagement."
