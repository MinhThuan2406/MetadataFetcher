#!/usr/bin/env python3
"""
Test script for actual 18 tools with all output formats.
Tests the codebase with all available output formats for each actual tool.
"""
...
def get_actual_tools():
    """Get list of actual 18 tools from classifications."""
...
def test_all_formats():
    """Test all formats for all actual tools."""
    print("🚀 Starting comprehensive format testing for actual tools...")
    print("=" * 70)
    
    # Load tool classifications
    tool_classifications = load_tool_classifications()
    if not tool_classifications:
        print("❌ Failed to load tool classifications")
        return
    
    # Get actual tools
    tools = get_actual_tools()
    print(f"📋 Testing {len(tools)} actual tools:")
    for tool in tools:
        category = tool_classifications.get(tool, ["Unknown"])[0]
    ...
    results_file = f"testresults/test_results_actual_tools_{timestamp}.json"
