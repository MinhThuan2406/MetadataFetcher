...
# from metadata.fetchers.generic_fetcher import TOOL_CLASSIFICATION, normalize_tool_name, auto_fetch_product_info
from export_to_word import export_product_docx
# from metadata.fetchers.creative_media_fetcher import fetch_creative_media_metadata, TOOL_FALLBACKS
...
def process_creative_media_tools():
    ...
    for tool in tools:
        ...
        # Save as JSON
        output_dir = os.path.join("CodeOutputs", "metadata")
...
for tool_name_input in BATCH_TOOL_LIST:
    ...
    json_fallback_path = os.path.join("CodeOutputs", "metadata", f"{tool_name}.json")
